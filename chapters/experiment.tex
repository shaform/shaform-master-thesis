...
is the most powerful feature set. Combining all features yield the best results.

\input{tables/recognition-features}

We use Scikit-Learn library~\citep{scikit-learn} to experiment with
several different learning models with default parameters using all features.
...
we take the union of all connective component the resulting connective candidates
...
As there remain many overlapped candidates, the precision for connectives instances
...
baseline model already achieve an F1 of 87.97\%. The greedy
...
In additional, it has the advantage of simpler computation since the same Logistic
...
compare with various related work. The two models are discourse disambiguation
